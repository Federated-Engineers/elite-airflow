from datetime import datetime, timedelta

from airflow import DAG
from airflow.providers.standard.operators.python import PythonOperator

from business_logic.hg_data.hg_spreadsheet_s3 import (LANCY_SHEET_ID,
                                                      RHONE_SHEET_ID,
                                                      write_sheet_to_s3)

default_args = {
    "owner": "federatedengineers",
    "retries": 2,
    "retry_delay": timedelta(seconds=5),
}


dag = DAG(
    dag_id="hg_google_sheets_to_s3",
    default_args=default_args,
    schedule="0 8 * * *",
    description="Send Lancy and Rhone Google Sheets data to S3",
    start_date=datetime(2026, 4, 1),
    catchup=False,
    tags=["hg", "ingestion", "s3"],
)


write_lancy_s3 = PythonOperator(
    task_id="write_lancy_s3",
    python_callable=write_sheet_to_s3,
    op_kwargs={
        "sheet_id": LANCY_SHEET_ID,
        "dataset_name": "lancy",
        "table_name": "raw_lancy",
    },
    dag=dag,
)


write_rhone_s3 = PythonOperator(
    task_id="write_rhone_s3",
    python_callable=write_sheet_to_s3,
    op_kwargs={
        "sheet_id": RHONE_SHEET_ID,
        "dataset_name": "rhone",
        "table_name": "raw_rhone",
    },
    dag=dag,
)


[write_lancy_s3, write_rhone_s3]
