...
USING (
    SELECT
        ...
        $1:_sheet_row_number::NUMBER AS sheet_row_number,
    ...
    FROM @{{ params.bronze_schema }}.{{ params.stage_name }}/{{ ti.xcom_pull(task_ids=params.snapshot_task_id) }}/
         (FILE_FORMAT => '{{ params.bronze_schema }}.GSHEET_JSON_ARRAY_FORMAT')
) AS source
...
WHEN MATCHED AND TO_JSON(target.RAW_RECORD) != TO_JSON(source.raw_record)
    THEN UPDATE SET
        ...
        SHEET_ROW_NUMBER   = source.sheet_row_number,
